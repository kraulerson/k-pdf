...
The Claude Dev Framework (github.com/kraulerson/claude-dev-framework) is a hook-based system that mechanically enforces coding standards, security scanning, and documentation requirements through Git hooks. It operates on a Swiss cheese defense model — layered controls where no single check catches everything, but the combination covers the gaps.
...
The Builder's Guide defines quality controls at each phase. The Claude Dev Framework enforces them mechanically:
